...
<div class="quanlysp">
	...
	<p> Hiện có <font color=red><b><?php echo $dem ?></b></font> tài khoản</p>
</div>
<table>
...
?>
            <tr class='noidung_hienthi_sp'>
                ...
                <td class="active_hienthi_sp">
                    <a href='?admin=suand&idnd=<?php echo $bien['idnd'] ?>'><img src="img/edit.png" title="Sửa"></a>
					<?php echo "<p onclick = 'checkdel({$bien['idnd'] })' ><img src='img/delete.png' title='Xóa' class='delete'></p>" ?>
                </td>
            </tr>
<?php 
    }
...
?>
	...
	<?php
			...
			if($page > 1){  
			$prev = ($page - 1);  
			echo "<a href=\"".$_SERVER['PHP_SELF']."?admin=showaccounts&page=$prev\"><button class='trang'>Trang trước</button></a>&nbsp;";
			}
			for($i = 1; $i <= $total_pages; $i++){  
			if(($page) == $i){  
			echo "$i&nbsp;";  
			} else {  
			echo "<a href=\"".$_SERVER['PHP_SELF']."?admin=showaccounts&page=$i\"><button class='so'>$i</button></a>&nbsp;";
			}  
			}  
			...
			if($page < $total_pages){  
			$next = ($page + 1);  
			echo "<a href=\"".$_SERVER['PHP_SELF']."?admin=showaccounts&page=$next\"><button class='trang'>Trang sau</button></a>";
			}  
			echo "</center>";  		
...
<script language="JavaScript">
    ...
    {
        var	idnd=idnd;
        var link="deleteaccount.php?idnd="+idnd;
        if(confirm("Bạn muốn xoá tài khoản này?")==true)
